...
class TextProcessing:
	def __init__(self):
		...
		self.classifier = pipeline("zero-shot-classification", model='MoritzLaurer/mDeBERTa-v3-base-mnli-xnli', tokenizer='MoritzLaurer/mDeBERTa-v3-base-mnli-xnli')
		print("cls loaded")
		self.ner = pipeline("ner", grouped_entities=True, model='xlm-roberta-large-finetuned-conll03-english', tokenizer='xlm-roberta-large-finetuned-conll03-english')
		print("ner loaded")
	...
	def classify_topic(self):
		...
		classifier_result = self.classifier(self.text_string, topic_cats, multi_label=False) # по идее можно в конец перенести и использовать с отдельными словами мб как-то на качество повлияет
	...
	def create_lemmatization_dict(self):
		...
		self.sentence_lemmat = extract_lemma(self, self.text_string) # " ".join([word.title() for word in sentences_lemmat[0]])
